...
BEGTEST(cmd_new_1){
  char *argv[] = {"cat","quote.txt","gettysburg.txt",NULL};
  ...
  if( assert_str_equals("cmd->argv[1] wrong","quote.txt",cmd->argv[1]) ) return;
  if( assert_str_equals("cmd->argv[2] wrong","gettysburg.txt",cmd->argv[2]) ) return;
...
BEGTEST(cmd_new_2){
  char *argv[] = {"grep","-i", "the ","gettysburg.txt",NULL};
  cmd_t *cmd = cmd_new(argv);
  ...
  if( assert_str_equals("cmd->argv[3] wrong","gettysburg.txt",cmd->argv[3]) ) return;
...
BEGTEST(read_all_1){
  ...
  int result = close(fd);
  if( result == -1 ){ fail("fd already closed, should not be"); return; }
...
BEGTEST(cmd_update_3){
  char *argv[] = {"grep","-i", "shmeckles","gettysburg.txt",NULL};
  cmd_t *cmd = cmd_new(argv);
  char *expect_output =
    "";
  cmd_start(cmd);
...
BEGTEST(cmdctl_update_1){
  ...
  catch_stdout();
  cmdctl_update_state(ctl,DOBLOCK);
  restore_stdout();
  char expect_alert[BUFSIZE];
  char *fmt =
    "@!!! cat[#%d]: EXIT(0)\n"
...
BEGTEST(cmdctl_print_1){
  ...
  snprintf(expect_start, BUFSIZE, expect_start_fmt,
           ctl->cmd[0]->pid, ctl->cmd[1]->pid, ctl->cmd[2]->pid);
