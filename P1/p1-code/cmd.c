...
// Initializes the components of a cmd
cmd_t *cmd_new(char *argv[]){
  cmd_t *cmd = malloc(sizeof(cmd_t));									           // Allocates necessary memory for the struct
  int i = 0;
  while(argv[i] != NULL){										                     // Copies each argument of argv to cmd->argv using strdup
...
// Frees all of the memory that was allocated to create the cmd, cmd arguments
// and cmd output.
...
// Starts running a cmd process and redirects input and output to the pipe.
void cmd_start(cmd_t *cmd){
  ...
  cmd->pid = fork();											                     // Sets the process as a child
	if (cmd->pid == 0){										                       // Redirects the child's write output from the screen to the pipe
		...
		dup2(cmd->out_pipe[PREAD],PREAD);							             // Redirects the parent's read input from the keyboard to the pipe
		close(cmd->out_pipe[PWRITE]);								               // Closes the pipe's write output on the parent's end
	}
}

// Reads all of bytes in the file associated with the file descriptor fd.
...
// Prints to STDOUT_FILENO
void cmd_print_output(cmd_t *cmd){
  if(cmd->output == NULL){										                        // Checks if process as generated output
    eprintf("%s[#%d] has no output yet\n", cmd->name, cmd->pid);			// Prints statement with information stating that no output has been generated
  }
  else{													                                      // At this point, the process has generated output
    write(STDOUT_FILENO, cmd->output, cmd->output_size);						  // Writes output to STDOUT_FILENO from cmd->output
  }
}

// Updates the state of the cmd argument using the nohang argument
// as the options argument to waitpid
